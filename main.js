 ...
 function fancyTimeFormat(time) //function for displaying duration of song in proper format
   {   
	...
	changeCurrentSongDetails(songs[0]);    //shows the details of first song in the playlist after loading the window
		$('#songs').DataTable({
			paging : false
		});	
	...
	var songName = songObj.fileName; // New Variable 
		$(id).click(function() {
			...
			if(songNunmber !== position)
			{
				audio.src=songName;
				songNunmber=position;   //updating songNunmber with new value
				changeCurrentSongDetails(songObj);   // Function Call to change the details of current song
			}
            toggleSong();
		});
	}
